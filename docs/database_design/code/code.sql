...
CREATE TABLE IF NOT EXISTS `quizapp`.`User` (
  ...
  `email_verification_token` VARCHAR(40) NULL COMMENT '',
  `email_verification_date` TIMESTAMP NULL COMMENT '',
  `password_recovery_token` VARCHAR(40) NULL COMMENT '',
  `password_recovery_date` TIMESTAMP NULL COMMENT '',
  `new_email` VARCHAR(50) NULL COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  INDEX `alvl_idx` (`access` ASC)  COMMENT '',
  UNIQUE INDEX `email_verification_token_UNIQUE` (`email_verification_token` ASC)  COMMENT '',
  UNIQUE INDEX `password_recovery_token_UNIQUE` (`password_recovery_token` ASC)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `quizapp`.`Questionnaire` (
  ...
  `message_required` INT NOT NULL DEFAULT 0 COMMENT '',
  `creation_date` TIMESTAMP NOT NULL COMMENT 'Questionaire last updated time.',
  PRIMARY KEY (`id`)  COMMENT '',
  INDEX `uid_idx` (`coordinator_id` ASC)  COMMENT '',
  UNIQUE INDEX `name_UNIQUE` (`name` ASC)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `quizapp`.`ExaminerApplication` (
  ...
  `accepted` TINYINT(1) NULL DEFAULT 0 COMMENT '',
  `date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '',
  `application_text` VARCHAR(255) NOT NULL COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `quizapp`.`QuestionnaireRequest` (
  ...
  `request_type` INT NOT NULL COMMENT '',
  `request_text` VARCHAR(255) NULL COMMENT '',
  `request_date` TIMESTAMP NOT NULL COMMENT '',
  `response_text` VARCHAR(255) NULL COMMENT '',
  `accepted` INT(1) NULL COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  INDEX `request_user_fk_idx` (`user_id` ASC)  COMMENT '',
  INDEX `request_questionnaire_fk_idx` (`questionnaire_id` ASC)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `quizapp`.`QuestionnaireParticipation` (
  `user_id` INT NOT NULL COMMENT '',
  `questionnaire_id` INT NOT NULL COMMENT '',
  `participation_type` INT NOT NULL COMMENT '',
  `participation_date` TIMESTAMP NOT NULL COMMENT '',
  PRIMARY KEY (`user_id`, `questionnaire_id`, `participation_type`)  COMMENT '',
  INDEX `participation_questionnaire_fk_idx` (`questionnaire_id` ASC)  COMMENT '',
