<?php
	...
	class UserMapper extends DataMapper
	{
		...
		public function findAvailableCoordinators($questionnaireId)
		{
			$query = "SELECT DISTINCT `User`.* 
						FROM `User`,`Questionnaire`
						WHERE `User`.`access`>=2 AND `Questionnaire`.`coordinator_id`<>`User`.`id` AND `Questionnaire`.`id`=?";
